...
// 대시보드 홈 셸: 상단 헤더(타이틀 + 로그아웃) + 본문 + 푸터. 관리 페이지는 AdminSidebarShell 을 쓴다.
export function AdminShell({ pageTitles }: AdminShellProps) {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background-1">
      <AdminHeader title={pageTitles[pathname] ?? ''} onLogout={handleLogout} />
